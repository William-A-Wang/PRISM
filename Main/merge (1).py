...
def sliding_window_regions(sequence: str, window_sizes: List[int], overlaps: int = 250) -> Dict[int, List[Dict]]:
    ...
    for window_size in window_sizes:
        ...
        while start < sequence_length:
            ...
            # **如果 `end` 已经达到 `sequence_length`，执行最后一次循环然后停止**
    ...
    for window_size, blocks in regions.items():
        ...
        for block in blocks:
            if block['id'] == max_diff_block:
                ...
                # block['end'] = min(end1 + 100, sequence_length)
                # block['start'] = min(start1 + 100, sequence_length)
                # block['id'] = f"size_1000_region_{block['start']}_{block['end']}"
            ...
            # ✅ **合并最后一个小 block**
            if block['end'] == sequence_length:
                print(f"Block {block['id']} reaches sequence end.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_result = iterative_block_optimization(sequence, regions, max_deviation=600)
...
# 提取所有的 pair_id 作为矩阵的行/列索引
pair_ids = [primer["pair_id"] for primers in final_result.values() for primer in primers]

print("Total Pair IDs:", len(pair_ids))
print("First 10 Pair IDs:", pair_ids[:10])
...
pair_id = [primer["pair_id"] for primers in primer_results.values() for primer in primers]
df_badness2 = pd.DataFrame(badness_matrix, index=pair_id, columns=pair_id)

print(df_badness2.head(10))
...
def approximation_algorithm(S, S_init, epsilon, badness_df, primer_results):
    ...
    S_opt1 = fast_local_search(S, S_init, epsilon, badness_matrix, primer_results)
...
S = list(range(len(badness_matrix)))
epsilon = 0.01
final_S_opt = approximation_algorithm(S, S_init, epsilon, badness_matrix, primer_results)
