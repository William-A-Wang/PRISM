...
input_file_path = "primer3_output.txt" #Your path
...
badness_df = New_Matrix(primer_df)
print(badness_df)
...
def greedy_primer_optimization(badness_df, delta_thres, debug=False):
    ...
    while True:
        ...
        for new_index in S:
            ...
            delta_temp = badness_df[new_index, new_index]

            for selected in S_init:
                delta_temp += badness_df[selected, new_index]
                delta_temp += badness_df[new_index, selected]
